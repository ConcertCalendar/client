...
function CalendarModal ({closeModal, title , content , pageXY}) {
    ...
    const popUpModalPosition = () => {
        if(800 < pageXY[1] && pageXY[1] < 1000){
            modalRef.current.style = `top : ${pageXY[1] - 675 }px; left: ${pageXY[0] - 475}px;`;
            setPosition("top");
        }else  if(1000 <= pageXY[1]){
            modalRef.current.style = `top : ${pageXY[1] - 1125 }px; left: ${pageXY[0] - 475}px;`;
        }
    }
